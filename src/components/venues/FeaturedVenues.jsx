...
const FeaturedVenues = () => {
  ...
  useEffect(() => {
    const fetchVenues = async () => {
      try {
        ...
        const topRatedVenues = response.data.filter(venue => venue.rating === 5);
        const randomTopRatedVenues = topRatedVenues.sort(() => 0.5 - Math.random()).slice(0, 4);
        setVenues(randomTopRatedVenues);
        setIsLoading(false);
      } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      <h2 className="text-3xl font-light mb-6">Featured Venues</h2>
